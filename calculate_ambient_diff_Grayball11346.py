...
real_rgb_list = scipy.io.loadmat('real_illum_11346.mat')['real_rgb']
image_list_file = open('Grayball11346/file.lst')

import csv
data_file = open('ambient_diff_Grayball11346.csv', 'w', newline='')
data_filednames = [
    'file',
    'ground_truth',
    'ambient_mean',
...
for f, real_rgb in zip(image_list_file, real_rgb_list):
    basename = f.replace('Source_Image/','') \
                .replace('\n', '')
    proceed_rgb_mat = np.load('Grayball11346_data/' +
        basename.replace('.jpg', '.npy'))
